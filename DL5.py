...
class DLLayer(object):
    ...
    def init_weights(self, W_initialization):
        ...
        if W_initialization == "zeros":
            ...
            prev_l = self._input_shape
            self.W = np.random.randn(*self._get_W_shape()) * np.sqrt(1.0 / sum(prev_l))
        elif W_initialization == "He":
            prev_l = self._input_shape
            self.W = np.random.randn(*self._get_W_shape()) * np.sqrt(2.0 / sum(prev_l))
        else:
            try:
...
class DLModel(object):
    ...
    def compute_cost(self, AL, Y):
        ...
        for i in range(1, len(self.layers)):
            if self.layers[i].L2_lambda != 0:
                sum += self.L2_lambda * np.sum(np.square(self.layers[i].W)) / (2*m)
        J = (np.sum(errors) / m) + sum
        return J

    def train(self, X, Y, num_epocs, mini_batch_size):
        self.set_train(True)
        ...
        for i in range(num_epocs):
            ...
            for b in mini_batches:
                Al = np.array(b[0], copy=True)
                # forward propagation
                Al = self.forward_propagation(Al)
                #backward propagation
                dAl = self.backward_propagation(Al,b[1])
			#record progress
            if num_epocs == 1 or (i > 0 and i % print_ind == 0):
                J = self.compute_cost(Al, b[1])
                costs.append(J)
                inject_string = ""
        ...
        #costs.append(self.compute_cost(Al, b[1]))
        self.set_train(True)
        return costs
    ...
    def random_mini_batches(self, X, Y, mini_batch_size=64, seed=0):
        ...
        num_complete_minibatches = math.floor(m/mini_batch_size) # number of complete mini batches
        mini_batches = []
